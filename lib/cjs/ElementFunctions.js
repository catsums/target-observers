...
var import_vector2 = require("@catsums/vector2");
var import_my = require("@catsums/my");
var import_svg_path_parser = __toESM(require("svg-path-parser"));
...
function getAnchor(elem, opts = { global: false }) {
  ...
  let offset = new import_vector2.Vector2(extents);
    ...
    switch (tOrigin[i].toLowerCase()) {
      ...
      default:
        if (tOrigin[i].endsWith("%")) {
          ...
          val = (0, import_my.getCSSValueInPixels)(tOrigin[i]);
        }
        if (i == 0)
          offset.x = val;
        if (i == 1)
...
function getLocalOffsetRect(elem) {
  ...
  if (elem instanceof HTMLElement) {
    localRect = new import_vector2.Rect2(
      elem.offsetLeft,
      elem.offsetTop,
      elem.offsetWidth,
      elem.offsetHeight
...
function getGlobalBoundingRect(elem) {
  ...
  let globalRect = new import_vector2.Rect2(
    _rect.x,
    _rect.y,
    _rect.width,
    _rect.height
...
function getLocalBoundingRect(elem) {
  ...
  let parentRect = new import_vector2.Rect2(import_vector2.Vector2.ZERO, import_vector2.Vector2.ZERO);
  switch (positionType) {
    case "static":
    case "relative":
    case "absolute":
    case "sticky":
      {
        ...
        if (parent) {
          ...
          parentRect = new import_vector2.Rect2(
            prect.x,
            prect.y,
            prect.width,
            prect.height
  ...
  let localRect = new import_vector2.Rect2(
    import_vector2.Vector2.SUBTRACT(globalRect.position, parentRect.position),
    globalRect.size
  );
  return localRect;
}
...
function roundTo(num, step) {
  if (step == 0)
    return num;
  if ((0, import_my.isInfinity)(step))
    return Infinity;
...
function decomposeMatrix(matrix, _t = 0) {
  ...
  let ky = Math.tan(0);
  return {
    translate: new import_vector2.Vector2(roundTo(tx, _t), roundTo(ty, _t)),
    scale: new import_vector2.Vector2(roundTo(sx, _t), roundTo(sy, _t)),
    skew: new import_vector2.Vector2(roundTo(kx, _t), roundTo(ky, _t)),
...
function transformCSSCoord(transformArr, v) {
  ...
  if (transformArr.length == 6) {
    ...
    return new import_vector2.Vector2({
      x: (0, import_my.safeDivide)(x * t[3] - y * t[2] + t[2] * t[5] - t[4] * t[3], det),
      y: (0, import_my.safeDivide)(-x * t[1] + y * t[0] + t[4] * t[1] - t[0] * t[5], det)
    });
  } else {
    return v;
  }
...
function getElemTransformFromCSSStyle(elem, _t = 0) {
  ...
  if (cssTranslate.length == 1) {
    ...
    if (isNaN(_x))
      _x = 0;
    p = new import_vector2.Vector2(_x, 0);
  } else {
    let _x = parseFloat(compStyle.translate.split(" ")[0]);
    ...
    if (isNaN(_y))
      _y = 0;
    p = new import_vector2.Vector2(_x, _y);
  }
  ...
  if (cssScale.length == 1) {
    ...
    if (isNaN(_x))
      _x = 1;
    s = new import_vector2.Vector2(_x, _x);
  } else {
    let _x = parseFloat(compStyle.scale.split(" ")[0]);
    ...
    if (isNaN(_y))
      _y = 0;
    s = new import_vector2.Vector2(_x, _y);
  }
  p = new import_vector2.Vector2(roundTo(p.x, _t), roundTo(p.y, _t));
  r = roundTo(r, _t / 10);
  s = new import_vector2.Vector2(roundTo(s.x, _t), roundTo(s.y, _t));
  let transform = new import_vector2.Transform2(
    p,
    r,
    s,
    import_vector2.Vector2.ZERO,
    anchor
  );
  return transform;
}
...
function getElemTransformFromMatrix(elem, precision = 0) {
  ...
  let rect = getLocalOffsetRect(elem);
  let anchor = getAnchor(elem);
  let transform = new import_vector2.Transform2(p, r, s, k, anchor);
  return transform;
}
...
function getElemPointsBasedOnBasicShape(elem, shape, props = [], { transform = true, global = true, opts = {} }) {
  ...
  if (transform) {
    ...
    mTransform = new import_vector2.Transform2();
    sTransform = new import_vector2.Transform2();
  }
  let points = [];
  switch (shape) {
    case "inset":
      points = getPointsInset(rect, props, opts);
      points.push(new import_vector2.Vector2(points[0]));
      break;
    case "circle":
    case "ellipse":
...
function getPointsPath(r, props = [], { increment = 0.125, vertices = 8 }) {
  ...
  let rect = new import_vector2.Rect2(r);
  let pts = [];
  let paths = [];
  for (let prop of props) {
    ...
    switch (prop.code) {
      ...
      case "L":
        {
          let pt = new import_vector2.Vector2(prop.x, prop.y);
          pts.push(pt);
        }
        break;
      case "S":
      case "C":
        {
          ...
          let _pts = import_vector2.Vector2.quadraticBezierPoints([
            new import_vector2.Vector2(prop.x0, prop.y0),
            new import_vector2.Vector2(prop.x1, prop.y1),
            new import_vector2.Vector2(prop.x2, prop.y2),
            new import_vector2.Vector2(prop.x, prop.y)
          ], increment);
          pts = pts.concat(_pts);
        }
        break;
  ...
  for (let path of paths) {
    for (let _pt of path) {
      newPts.push(new import_vector2.Vector2(_pt));
    }
  }
  let t1 = new import_vector2.Transform2(new import_vector2.Vector2(rect.left, rect.top));
...
function getPointsPolygon(r, props = [], opts) {
  ...
  for (let prop of props) {
    ...
    let x = rect.left + _x;
    let y = rect.top + _y;
    pts.push(new import_vector2.Vector2(x, y));
  }
  return pts;
}
...
function getPointsEllipse(r, props = [], { vertices = 8 }) {
  ...
  for (let i = 0; i < vertexCount; i++) {
    ...
    let pt = new import_vector2.Vector2(
      rx * Math.cos(angl),
      ry * Math.sin(angl)
    );
    pts.push(pt);
  }
  let t0 = new import_vector2.Transform2(new import_vector2.Vector2(cx, cy));
...
function getPointsInset(_rect, props = [], opts) {
  ...
  let rect = new import_vector2.Rect2(_rect);
  let t = 0, l = 0, r = 0, b = 0;
  let vals = [];
  for (let i = 0; i < props.length; i++) {
    let prop = props[i];
  ...
  let newPts = new import_vector2.Rect2(l, t, r - l, b - t).getCorners();
  return newPts;
}
__name(getPointsInset, "getPointsInset");
function isPointInsidePolygon(polygon, vec) {
  let arr = polygon.map((pt) => new import_vector2.Vector2(pt));
  let len = arr.length;
  let v = new import_vector2.Vector2(vec);
  let rect = import_vector2.Rect2.from(arr);
  if (!rect.containsPoint(v)) {
    return false;
  }
  let castLine = new import_vector2.Vector2Line(
    new import_vector2.Vector2(v),
    new import_vector2.Vector2(v.x + 1, v.y)
  );
  let intersects = [];
  for (let i = 0; i < len; i++) {
    let a = new import_vector2.Vector2(arr[i]);
    let b = new import_vector2.Vector2(arr[i + 1]);
    let line = new import_vector2.Vector2Line(a, b);
    if (line.hasPoint(v))
      return true;
    let intersect = line.intersect(castLine);
    if (intersect && intersect.x > v.x) {
      let rectAB = import_vector2.Rect2.from([a, b]);
...
function getPolygonIntersect(pA, pB, precision = Number.EPSILON) {
  if (pA.every((v) => v instanceof Array)) {
    ...
    let rectA = import_vector2.Rect2.from(pA);
    let rectB = import_vector2.Rect2.from(pB);
    let intersectRect = rectA.getIntersectWith(rectB, precision);
    if (!intersectRect)
      return null;
    let intersects = [];
    for (let i = 0; i < pA.length; i++) {
      let aa = new import_vector2.Vector2(pA[i]);
      let ab = new import_vector2.Vector2(pA[(i + 1) % pA.length]);
      let lineA = new import_vector2.Vector2Line(aa, ab);
      let lineARect = import_vector2.Rect2.from([aa, ab]);
      for (let j = 0; j < pB.length; j++) {
        let ba = new import_vector2.Vector2(pB[j]);
        let bb = new import_vector2.Vector2(pB[(j + 1) % pB.length]);
        let lineB = new import_vector2.Vector2Line(ba, bb);
        let lineBRect = import_vector2.Rect2.from([ba, bb]);
...
function fixPolygonPointsOrder(polygon, precision = Number.EPSILON) {
  let arr = polygon.map((pt) => new import_vector2.Vector2(pt)).slice();
  let len = arr.length;
  let rect = import_vector2.Rect2.from(arr);
  let swaps = [];
  mainLoop:
          ...
          loopB:
            for (let j = 0; j < len; j++) {
              ...
              if (!intersectAB)
                continue;
              for (let pt of arr) {
                if (import_vector2.Vector2.EQUALS(pt, intersectAB, precision)) {
...
function sortPoints(points) {
  points = points.slice();
  let p0 = new import_vector2.Vector2();
