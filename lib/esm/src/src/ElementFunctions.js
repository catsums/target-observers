...
export function getAnchor(elem, opts={global:false}){
...
export function getGlobalOffsetRect(elem){
...
export function getLocalOffsetRect(elem){
	let localRect;
...
export function getGlobalBoundingRect(elem){
...
export function getLocalBoundingRect(elem){
...
export function deltaTransformPoint(matrix, point)  {
	...
	return { x: dx, y: dy };
}export function decomposeMatrix(matrix, _t=0) {
...
export function parseCSSTransform(transform){
	//add sanity check
	return transform.split(/\(|,|\)/).slice(1,-1).map( function(v){
		return parseFloat(v);
	});
}
export function transformCSSCoord(transformArr, v){
	v = new Vector2(v);
	let x = v.x, y = v.y;
...
export function getElemExpandedTransformFromMatrix(elem, _t=0){
...
export function getElemExpandedTransformFromCSSStyle(elem, _t=0){
...
export function getElemTransformFromCSSStyle(elem, _t=0){
	let compStyle = window.getComputedStyle(elem);
...
export function getElemTransformFromMatrix(elem,precision=0){
	let compStyle = window.getComputedStyle(elem);
...
export function getBasicShapeProps(cssValueString){
	//let ptrn = (/\((.*?)\)/g); ///anything inbetween brackets
	// let cssValueString = window.getComputedStyle(elem)['clip-path'];
...
export function getElemPointsBasedOnBasicShape(elem, shape, props, {transform=true, global=true, opts={}}){
	let rect;
...
export function getPointsPath(rect, props=[], {increment=0.125, vertices=8}){
	if(!MY.isArray(props)){
...
export function getPointsPolygon(rect, props=[], opts){
	if(!MY.isArray(props)){
		if(props) props = [props];
		else props = [];
	}
...
export function getPointsEllipse(rect, props=[], {vertices=8}){
	if(!MY.isArray(props)){
		if(props) props = [props];
		else props = [];
	}
	...
	let pts = []; let vertexCount = vertices ?? 10;
...
export function getPointsInset(rect, props=[], opts){
	if(!MY.isArray(props)){
		if(props) props = [props];
		else props = [];
	}
...
export function isPointInsidePolygon(polygon, v){
	let arr = polygon.slice();
	let len = arr.length;
	v = new Vector2(v);

	let rect = Rect2.from(polygon);
...
export function getPolygonIntersect(pA, pB, precision=Number.EPSILON){
	if( pA.every((v)=>(MY.isArray(v))) ){
		let rects = [];
		for(let _pA of pA){
			let _rect = getPolygonIntersect(_pA,pB);
			if(_rect){
				if(MY.isArray(_rect)) rects = rects.concat(_rect);
		...
		let rectA = Rect2.from(pA); let rectB = Rect2.from(pB);
		...
		for(let i=0; i<pA.length-1;i++){
			let aa = new Vector2(pA[i]); let ab = new Vector2(pA[i+1]);
			let lineA = new Vector2Line(aa,ab);
			let lineARect = Rect2.from([aa,ab]);

			for(let j=0; j<pB.length-1;j++){
				let ba = new Vector2(pB[j]); let bb = new Vector2(pB[j+1]);
